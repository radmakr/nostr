    ...
    C: Verification,
{
    ...
    let delegator_public_key = delegator_public_key.as_x_only()?;
...
mod tests {
    ...
    fn test_sign_delegation_verify_lowlevel() {
        ...
        let verify_result = SECP256K1.verify_schnorr(
            &signature,
            &message,
            delegator_keys.public_key().as_x_only().unwrap(),
        );
        assert!(verify_result.is_ok());
    }
    ...
    fn test_delegation_tag_to_json() {
        ...
        let delegator_sk =
            SecretKey::from_hex("b2f3673ee3a659283e6599080e0ab0e669a3c2640914375a9b0b357faae08b17")
                .unwrap();
        let delegator_pubkey = Keys::new_with_ctx(&secp, delegator_sk).public_key().clone();
